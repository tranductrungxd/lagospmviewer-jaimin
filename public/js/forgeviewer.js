...
var documentId =
  "urn:dXJuOmFkc2sud2lwcHJvZDpmcy5maWxlOnZmLlhfMTV4Z19TUS1ldDh3YUpfcVkxNVE_dmVyc2lvbj0x";
...
function getThreeLeggedToken(callback) {
  ...
  if (
    ...
    $.ajax({
      type: "POST",
      url: "https://developer.api.autodesk.com/authentication/v2/token?scope=data:read%20data:write%20data:create%20data:search%20code:all%20account:read%20user-profile:read%20viewables:read",
...
function loginToBim360() {
  ...
  if (
    tkn != null &&
    tkn != "undefined" &&
    typeof tkn != "undefined" &&
    getCookie("bimlogin") == "alive"
  ) {
 ...
 */
    ...
    var newWindow = window.open(
      url,
      "Logon to your BIM360 account.",
      "height=600,width=650,top=" + top + ",left=" + left
    );
...
BIM360IssueExtension.prototype = Object.create(
...
BIM360IssueExtension.prototype.onToolbarCreated = function (tools) {
...
BIM360IssueExtension.prototype.createUI = function () {
  ...
  {
    var createQualityIssues = new Autodesk.Viewing.UI.Button(
      "createQualityIssues"
    );
    createQualityIssues.onClick = function (e) {
      var pushPinExtension = _this.viewer.getExtension(
        _this.pushPinExtensionName
      );
      if (pushPinExtension == null) {
        ...
        _this.viewer
          .loadExtension(_this.pushPinExtensionName, extensionOptions)
          .then(function () {
            _this.createIssue();
          });
      } else _this.createIssue(); // show issues
    };
...
BIM360IssuePanel.prototype = Object.create(
...
BIM360IssueExtension.prototype.loadIssues = function (issueId) {
  ...
  this.pushPinExtensionName = "Autodesk.BIM360.Extension.PushPin";
  if (_this.panel == null) {
    _this.panel = new BIM360IssuePanel(
      _this.viewer,
...
BIM360IssueExtension.prototype.getIssues = function (issueId) {
  ...
  if (_this.issues.length > 0) {
    ...
    } else _this.showIssues(); // show issues
  } else {
    if (_this.panel)
      _this.panel.addProperty("No issues found", "Use create issues button");
  }
};
...
/*   pushPinExtension.addEventListener(
    "pushpin.selected",
    this.onSelectedChangedBind
  );
 */
  // var pushpinDataArray = [];
  $("#bim360IssuePanel-scroll-container > .treeview").prepend(
    "<button id='importIssueBtn' class='forgePanelButton importIssueBtn'>Import selected</button><button id='selectAllIssue' class='forgePanelButton selectAllIssueBtn'>Select All</button>"
  );
  _this.issues.forEach(function (issue) {
    ...
    if (pushpinAttributes) {
      ...
      delete viewer_state.attributesVersion
      ...
      if (viewer_state && viewer_state.seedURN != "undefined") {
        ...
        if (pushpinAttributes) {
          ...
          const location = pushpinAttributes.details.position;
...
BIM360IssueExtension.prototype.createIssue = function () {
  ...
  var mypanel = new BIM360CreateIssuePanel(
    viewer.container,
    "createIssuePanel",
    "Create Issue",
    content
...
async function getIssuesData (
  ...
  const issue = dumpIssue.itemData;
 
  const viewerState = issue.viewerState

  const payloadIssueId = await getIssueTypeId();

  if (payloadIssueId?.results?.length !== 0) {
    const issueTypeId = payloadIssueId.results.find(
      (f) => f.title === "Quality"
    );
    const issueSubtypeId = issueTypeId.subtypes.length
      ? issueTypeId.subtypes[0].id
      : null;
    const urn_id = "urn:adsk.wipprod:dm.lineage:X_15xg_SQ-et8waJ_qY15Q";
        ...
        linkedDocuments: [
          {
            ...
            details: {
              viewable: {
                name: issue.objectData.viewName,
                is3D: true,
                viewableId: issue.objectData.viewableId,
                guid: issue.objectData.guid,
              },
...
$(document).on("click", "#addWIR", function () {
  ...
  var pushPinExtension = viewer.getExtension(
  ...
  pushPinExtension.pushPinManager.addEventListener(
    ...
    async function (e) {
      ...
      const dummyIssue = e.value
      newIssueData = await getIssuesData(dummyIssue)
     
      if (newIssueData === null) return;
  ...
  pushPinExtension.startCreateItem({
    label: "New",
    status: "open",
    type: "issues",
  });
...
BIM360CreateIssuePanel.prototype = Object.create(
...
BIM360CreateIssuePanel.prototype.initialize = function () {
  ...
  var issueType =
    '<select id="issueType" class="form-control createIssueInputs"></select>';
  var issueSubType =
    '<select id="issueSubType" class="form-control createIssueInputs"></select>';

  var myvar =
    '<div style="padding: 5px 40px 5px 15px;"><div class="form-group"><label for="issueTitle">Title</label>' +
    ...
    '<div class="form-group"><label for="issueType">Issue type</label>' +
    issueType +
    '<small id="typeError" style="color:red; display:none;" class="form-text text-muted">Select valid type.</small></div>' +
    '<div class="form-group"><label for="issueType">Issue sub type</label>' +
    issueSubType +
    '<small id="subTypeError" style="color:red; display:none;" class="form-text text-muted">Select valid sub type.</small></div>' +
...
$(document).on("click", "#saveWir", function () {
  ...
  $.getJSON("/getLatestIdWir", async function (latest) {
    var newid;
    ...
    newIssueData.title = `${newid}`
  
 
    var urls =
      "https://developer.api.autodesk.com/issues/v2/containers/e79b1aa1-aeb6-40c7-9508-c35e4c7ec6c2/issues";
    ...
    $.ajax({
      ...
      beforeSend: function (request) {
        request.setRequestHeader(
          "Authorization",
          "Bearer " + localStorage.getItem("bimToken")
        );
        request.setRequestHeader("Content-Type", "application/vnd.api+json");
      },
      url: urls,
      async: false,
      data: JSON.stringify(newIssueData),
      error: function (httpObj, textStatus) {
        console.log(httpObj);
      },
      success: function (res) {
        ...
        console.log("issue created successfully. " + res.id);
      },
    });
...
$(document).on("click", ".mainWir", function () {
  ...
  if (id == null || id == "") {
    ...
    toastr.info("No issue linked to the WIR.");
  } else {
...
function processChecklist(data, wirid) {
  $.ajax({
    ...
    success: function (datas) {
      ...
      data.consent_proceed == 1
        ? $("#consentRead").attr("checked", "checked")
        : $("#consentRead").removeAttr("checked");
      data.consent_proceed_comment == 1
        ? $("#yesConsentRead").attr("checked", "checked")
        : $("#yesConsentRead").removeAttr("checked");
      data.no_consent == 1
        ? $("#noConsentRead").attr("checked", "checked")
        : $("#noConsentRead").removeAttr("checked");
      data.rev_not_req == 1
        ? $("#reviewNotRead").attr("checked", "checked")
        : $("#reviewNotRead").removeAttr("checked");
      ...
      if (data.inspection_type == "sbe") {
        $.each(list, function (k, v) {
          ...
          $("#asbe input[type=date]").each(function () {
            if (
              k == "so_date_site" &&
              $(this).attr("id") == "sitedateso" &&
              v != ""
            ) {
              $(this).val(list.so_date_site);
            } else if (
              k == "so_date_qc" &&
              $(this).attr("id") == "qcdateso" &&
              v != ""
            ) {
              $(this).val(list.so_date_qc);
            } else if (
              k == "so_date_eng" &&
              $(this).attr("id") == "engedateso" &&
              v != ""
            ) {
        ...
        $.each(list, function (k, v) {
          ...
          $("#aswd input[type=date]").each(function () {
            if (
              k == "so_date_site" &&
              $(this).attr("id") == "sitedateso1" &&
              v != ""
            ) {
              $(this).val(list.so_date_site);
            } else if (
              k == "so_date_qc" &&
              $(this).attr("id") == "qcdateso1" &&
              v != ""
            ) {
              $(this).val(list.so_date_qc);
            } else if (
              k == "so_date_eng" &&
              $(this).attr("id") == "engedateso1" &&
              v != ""
            ) {
              $(this).val(list.so_date_eng);
            }
...
function createIssueFromLPM(wirid, url) {
  ...
  pushPinExtension.pushPinManager.addEventListener(
    ...
    async function (e) {
      ...
      pushPinExtension.endCreateItem();
      const dumpIssue = e.value
  /*     var issue = pushPinExtension.getItemById(
        pushPinExtension.pushPinManager.pushPinList[0].itemData.id
      ); */
      const newIssueData = await getIssuesData(dumpIssue)
    
      if (newIssueData === null) return;

      newIssueData.title = parseInt(wirid) + 2000;
      var urls =
        "https://developer.api.autodesk.com/issues/v2/containers/e79b1aa1-aeb6-40c7-9508-c35e4c7ec6c2/issues";
...
function applyColorsToModel(dbIdOfNode, applyStatus, applyviewer) {
  ...
  if (applyStatus == 0) {
    ...
    console.log(
      "DEBUG Color - no color for given status - node status:" + applyStatus
    );
  }
  applyviewer.setThemingColor(
    dbIdOfNode,
...
function startPlannedVsActual() {
  ...
  if (
    !$.isEmptyObject(viewer) &&
    !$.isEmptyObject(viewer.getVisibleModels()[0])
  ) {
    console.log("dhoom");
    ...
    Autodesk.Viewing.Initializer(options, () => {
      ...
      comparisonViewer = new Autodesk.Viewing.Private.GuiViewer3D(div, {
      ...
      if (!$.isEmptyObject(comparisonViewer)) {
        ...
        comparisonViewer.addEventListener(
          ...
          function () {
            if (!$.isEmptyObject(comparisonViewer)) {
              ...
              if (!compareViewports(v1state, v2state)) {
                viewerFlick = 2;
                viewer.restoreState(
                  comparisonViewer.getState(sfilter),
                  sfilter,
                  false
                );
          ...
          function () {
            if (!$.isEmptyObject(comparisonViewer)) {
              ...
              if (!compareViewports(v1state, v2state) && viewerFlick == 0) {
                viewerFlick = 1;
                comparisonViewer.restoreState(
                  viewer.getState(sfilter),
                  sfilter,
                  false
                );
        ...
        comparisonViewer.addEventListener(
          ...
          function (event) {
            ...
            if (!$.isEmptyObject(comparisonViewer) && !$.isEmptyObject(e)) {
              ...
              if (
                e &&
                typeof e[0].dbIdArray != "undefined" &&
                viewerClick == 0
              ) {
                viewerClick = 2;
                for (
                  var i = 0;
                  i < comparisonViewer.getVisibleModels().length;
                  i++
                ) {
                  if (
                    comparisonViewer.getVisibleModels()[i].getModelId() ==
                    e[0].model.getModelId()
                  ) {
                    selections.push({
      ...
      Autodesk.Viewing.Document.load(secondUrn, (doc) => {
        ...
        comparisonViewer
          .loadDocumentNode(doc, viewables, modelOption)
          .then((model1) => {
...
function compareViewports(stateA, stateB) {
  ...
  if (
    viewportA["aspectRatio"] !== viewportB["aspectRatio"] &&
    viewportA["distanceToOrbit"] !== viewportB["distanceToOrbit"]
  ) {
    return false;
  }
...
function resizeAndShowHideEventsForPlannedVsActualOn() {
  ...
  $(`#forgeViewer2 > .adsk-viewing-viewer`).css({
    width: "50%",
    "border-right": "2px solid grey",
  });
  $(`#comparisionRange`).show().val("50");
  $(
    `#forgeViewer div#guiviewer3d-toolbar, #forgeViewer2 div#guiviewer3d-toolbar`
  )
    .addClass("adsk-toolbar-vertical")
    .css({ right: "unset", left: "10px" });
  ...
  $(
    `#forgeViewer2 #allPlannedVsActualExtensionToolbar,#forgeViewer #allPlannedVsActualExtensionToolbar`
  ).remove();
  $(
    `#forgeViewer2 #guiviewer3d-toolbar,#forgeViewer #guiviewer3d-toolbar`
  ).prepend(element);

  setTimeout(() => {
    $(`#forgeViewer .adsk-viewing-viewer .adsk-toolbar.adsk-toolbar-vertical .adsk-button>.toolbar-vertical-group,
        #forgeViewer2 .adsk-viewing-viewer .adsk-toolbar.adsk-toolbar-vertical .adsk-button>.toolbar-vertical-group`).css(
      { right: "unset", left: "7vh" }
    );

    $(
      `.adsk-viewing-viewer .adsk-toolbar.adsk-toolbar-vertical .adsk-button>.toolbar-vertical-group~.adsk-control-tooltip`
    ).css({ left: "8vh", right: "unset", bottom: 20 });

    $(`#forgeViewer`).append(
      `<div class="liveButtonDiv"><img class="liveButton" alt="live" src="app/images/Live.gif" /></div>`
    );
    $(`#forgeViewer2`)
      .append(`<div class="forgeInfoButtonDiv"><button class="btn-new forgeInfoButton" id="forgeInfoButton">Planned <i  class="fa fa-info-circle fa-2" aria-hidden="true"></i></button>
